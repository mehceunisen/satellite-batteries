...
charge_C_rate = 2; % Charging rate in C-rate (e.g., 1C means full charge in 1 hour)
discharge_C_rate = 1; % Discharging rate in C-rate (e.g., 0.5C means full discharge in 2 hours)
...
for i = 1:length(time)
    ...
    if mod(time(i), 60) < 30 % Example: charge for 30 minutes every hour
        % Charging
        SOC = SOC + (charge_C_rate * capacity_Ah * (time_step / 60))  / capacity_Ah * 100;
    else
        % Discharging
